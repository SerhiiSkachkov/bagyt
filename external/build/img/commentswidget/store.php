...
use uti\AdminCatalog\models\ProductRatios;
use uti\AdminCatalog\models\ProductsSettings;

?>
<?php
  $userRatio = ProductRatios::getUserRatioForProduct($object_id);
  $productsSettings = ProductsSettings::getSettings();
 ?>
...
<? endif; ?>
    ...
    <script type="text/template" id="answer_template">
        ...
        <? if ((Captcha::captchaConditions('reCAPTCHA', 'comments_news', (int)TRUE) && $object_alias == 'news') || (Captcha::captchaConditions('reCAPTCHA', 'comments_product', (int)TRUE) && $object_alias == 'product') || (Captcha::captchaConditions('reCAPTCHA', 'comments_blog_posts', (int)TRUE) && $object_alias == 'blog_posts')): ?>
            ...
            <div class="form-group"
                 style="min-height: 40px; overflow: hidden; <?= ($captcha_settings && $captcha_settings->alias == 'reCAPTCHA') ? '' : 'display:none;' ?>">
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.html) {
                                ...
                                <? if(Captcha::captchaConditions('reCAPTCHA', 'comments_product', (int)TRUE) && $object_alias == 'product'): ?>
